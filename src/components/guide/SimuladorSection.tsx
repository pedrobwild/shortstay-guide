    ...
    <SectionBlock id="simulador" title="Simulador de Receita" takeaway="Calcule sua rentabilidade estimada em menos de 1 minuto.">
      ...
      <Card className="border-border">
        <CardContent className="p-6 space-y-5 font-body">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="text-sm font-medium text-foreground mb-1 block">Diária atual (opcional, R$)</label><Input type="number" placeholder={`Média do bairro: R$ ${fmt((selected.dailyMin + selected.dailyMax) / 2)}`} value={simDiariaAtual} onChange={(e) => setSimDiariaAtual(e.target.value)} /></div>
            <div><label className="text-sm font-medium text-foreground mb-1 block">Objetivo</label><Select value={simObjetivo} onValueChange={setSimObjetivo}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="maximizar">Maximizar receita</SelectItem><SelectItem value="estabilidade">Estabilidade de ocupação</SelectItem><SelectItem value="premium">Posicionamento premium</SelectItem></SelectContent></Select></div>
          </div>
          <div><label className="text-sm font-medium text-foreground mb-2 block">Impacto de aumento na diária</label><div className="flex gap-2 flex-wrap">{[0, 10, 20, 30].map((v) => <Button key={v} size="sm" variant={rateBoost === v ? "default" : "outline"} onClick={() => setRateBoost(v)} className={rateBoost === v ? "bg-primary text-primary-foreground" : ""}>{v === 0 ? "Base" : `+${v}%`}</Button>)}</div></div>
          <div><label className="text-sm font-medium text-foreground mb-1 block">Orçamento de reforma (opcional, R$)</label><Input type="number" placeholder="Ex: 45.000" value={simReformaBudget} onChange={(e) => setSimReformaBudget(e.target.value)} /></div>
          <Separator />
